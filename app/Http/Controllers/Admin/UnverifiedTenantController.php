...
class UnverifiedTenantController extends Controller
{
    ...
    public function index()
    {
        ...
        $tenants = User::where(function ($q) use ($tenement) {
            $q->where('roles', UserRoles::TENANT->value)
                ->whereHas('tenant', function ($q) use ($tenement) {
    ...
    public function store(Request $request)
    {
        ...
        $message = [
            'url' => route('login'),
            'message' => "Your Pre Register is Accept Here your Account,  Email: {$user->email}  Password : password"
        ];
