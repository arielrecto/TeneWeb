<x-dashboard.super-admin.base>
    ...
    <div class="panel">
        <form action="{{route('super-admin.tenements.store')}}" method="post" class="flex flex-col gap-2 w-full p-2" enctype="multipart/form-data">
            @csrf
            ...
            <div class="w-full h-auto flex justify-center" x-data="imagePreview">
                ...
                <div class="flex items-center justify-center w-5/6" x-show="!imageSrc">
                    <label for="dropzone-file"
                        ...
                        <input id="dropzone-file" name="image" @change="uploadImageHandler($event)" type="file" class="hidden" />
                    </label>
                </div>
            </div>
