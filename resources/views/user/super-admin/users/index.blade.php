    ...
    <div class="panel flex flex-col gap-2 p-2">
        <div class="grid grid-cols-2 grid-flow-row  gap-2 h-32 w-full">
            @foreach ($userCounts as $userCount)
                <a href="{{$links[$userCount['name']] ? route($links[$userCount['name']]) : "#"}}" class="w-full rounded-lg shadow-lg p-2 capitalize flex flex-col justify-between">
                    <h1 class="font-bold text-primary">{{$userCount['name']}}</h1>
